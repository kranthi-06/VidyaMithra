                    ...
                    <div key={s.id} className="flex items-center">
                        <div className="flex flex-col items-center gap-1.5">
                            <motion.div
                                ...
                                className={`w-10 h-10 rounded-full flex items-center justify-center text-sm transition-all shadow-sm ${active ? 'bg-[#6366f1] text-white ring-4 ring-indigo-200' : done ? 'bg-emerald-500 text-white' : 'bg-gray-100 text-gray-400'}`}
                            >
                                {done ? <CheckCircle2 className="w-5 h-5" /> : <Icon className="w-4 h-4" />}
                            </motion.div>
                            <span className={`text-[9px] font-bold uppercase tracking-widest whitespace-nowrap ${active ? 'text-[#6366f1]' : done ? 'text-emerald-600' : 'text-gray-400'}`}>{s.label}</span>
                        </div>
                        {i < STEP_CONFIG.length - 1 && (
                            <div className={`w-8 sm:w-14 h-0.5 mx-2 mt-[-14px] transition-colors ${done ? 'bg-emerald-400' : 'bg-gray-100'}`} />
                        )}
                    </div>
                );
            })}
...
export function AIButton({ onClick, loading, label }: { onClick: () => void; loading: boolean; label?: string }) {
    return (
        <motion.button
            ...
            onClick={onClick}
            disabled={loading}
            className="px-6 py-3 bg-gradient-to-r from-violet-600 to-indigo-600 text-white rounded-2xl font-bold text-sm shadow-lg shadow-indigo-200 hover:shadow-xl transition-all disabled:opacity-60 flex items-center gap-2"
        >
            {loading ? (
        ...
        <div className="flex flex-wrap gap-2 pt-3 border-t border-gray-100 mt-4">
            ...
            {modes.map(m => (
                <button key={m.id} onClick={() => onRegenerate(m.id)} disabled={loading}
                    className="px-3 py-1.5 text-xs font-bold rounded-xl border border-gray-200 text-gray-600 hover:bg-indigo-50 hover:border-indigo-200 hover:text-indigo-700 disabled:opacity-40 transition-all">
                    {m.label}
                </button>
            ))}
        </div>
    ...
    return (
        <div className="space-y-1.5">
            ...
            <input type={type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder}
                className="w-full h-12 rounded-xl border border-gray-200 bg-gray-50/50 focus:bg-white focus:ring-2 focus:ring-indigo-200 focus:border-indigo-400 transition-all font-semibold px-4 text-sm outline-none" />
        </div>
    );
}
    ...
    return (
        <div className="space-y-1.5">
            ...
            <textarea value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} rows={rows}
                className="w-full rounded-2xl border border-gray-200 bg-gray-50/50 focus:bg-white focus:ring-2 focus:ring-indigo-200 focus:border-indigo-400 transition-all font-semibold p-4 text-sm outline-none resize-none" />
        </div>
    );
}
